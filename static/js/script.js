...
function listarPacotes() {
    ...
    fetch('/listar_pacotes', {
        ...
        .then(data => {
            ...
            listarScriptsBtn.disabled = true; // Habilita botão de listar scripts
        })
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        function listarPacotes() {
            ...
            fetch(`/listar_pacotes?dispositivo=${dispositivoId}`)
                .then(response => response.json())
                .then(data => {
                    pacotesSelect.innerHTML = '<option value="">Selecionar pacote</option>';
                    if (data.error) {
        ...
        function executarComando() {
            ...
            // Apenas adicionar o pacote se o modo não for "-F"
            if (forma !== '-F') {
                comandoLog.pacote = pacote;
            }
    
            appendLog(`Preparando comando: ${JSON.stringify(comandoLog)}`);
            console.log(`[DEBUG] Comando enviado ao backend:`, comandoLog);
    
            // Emitir o comando para o backend
        ...
        pacotesSelect.addEventListener('change', verificarExecutarBtn);
    });
